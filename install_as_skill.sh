...
##############################################################################
# YouTube Download - Claude Code Skill 安装脚本
#
# 功能：
# 1. 自动创建 Skill 目录
# 2. 复制所有必要文件
...
# 主函数
main() {
    print_header "YouTube download - Claude Code Skill 安装"

    # 1. 确定 Skill 目录
    SKILL_DIR="$HOME/.claude/skills/youtube-download"
    ...
    if [ -f "$SKILL_DIR/config.example.json" ]; then
        ...
        print_success "config.json 文件已创建"
        echo ""
        print_info "配置文件位置: $SKILL_DIR/config.json"
        print_info "如需自定义配置，可编辑："
        print_info "  nano $SKILL_DIR/config.json"
        print_info "  或"
        print_info "  code $SKILL_DIR/config.json"
    else
        print_warning "未找到 config.example.json 文件"
    fi

    # 11. 完成
    print_header "安装完成！"

    print_success "YouTube Downloader 已成功安装为 Claude Code Skill"
    echo ""
